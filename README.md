**Выполнен студенткой ИКБО-17-22**  
Цветкова Елена Александровна  
Проект по разработке кроссплатформенных программных систем  
Вариант: 23
...
python3 uvm_gui.py
```
### Сборка GUI с помощью PyInstaller
Для Mac (или других платформ с Python) можно собрать отдельное исполняемое приложение.
...
pyinstaller --name UVM_GUI --onefile --windowed uvm_gui.py
```
- `--name UVM_GUI` — имя создаваемого приложения
- `--onefile` — собрать всё в один исполняемый файл
- `--windowed` — запуск без консоли (только GUI)
3. После сборки:
- Откройте папку `dist/`
- Там будет файл `UVM_GUI` — это готовое приложение для запуска
4. Запуск GUI:
На Mac достаточно двойного клика по файлу UVM_GUI или через терминал:
```bash
./dist/UVM_GUI
```
## Примечания
